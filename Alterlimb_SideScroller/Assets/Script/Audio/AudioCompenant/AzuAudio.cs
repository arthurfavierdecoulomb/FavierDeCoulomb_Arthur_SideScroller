...
public class AzuAudio : MonoBehaviour
{
    ...
    readonly HashSet<SurfaceType> missingBankLogged = new HashSet<SurfaceType>();
    ...
    void Start()
    {
        ...
        if (GetBank(defaultSurface) == null)
            Debug.LogError($"[AzuAudio] Aucune Surface Bank pour la surface par defaut ({defaultSurface}) : toute detection qui echoue sera silencieuse.");
    ...
    SurfaceType DetectSurface()
    {
        ...
        if (TryReadTile(hit.collider, probePoint, false, out SurfaceType surface))
            return surface;

        tilemapCache.Remove(hit.collider);

        if (TryReadTile(hit.collider, probePoint, true, out surface))
            return surface;
        ...
        {
            if (logSurfaceDetection)
                Debug.Log($"[AzuAudio] {hit.collider.name} -> {colliderTag.surface} (tag du collider)");

            return colliderTag.surface;
        }

        if (logSurfaceDetection)
            Debug.LogWarning($"[AzuAudio] {hit.collider.name} : aucune tuile, aucun SurfaceTag -> surface par defaut ({defaultSurface}).");

        return defaultSurface;
    }

    bool TryReadTile(Collider2D collider, Vector3 probePoint, bool isRetry, out SurfaceType surface)
    {
        surface = defaultSurface;

        Tilemap[] tilemaps = ResolveTilemaps(collider);
        ...
        for (int i = 0; i < tilemaps.Length; i++)
        {
            ...
            {
                if (logSurfaceDetection)
                    Debug.Log($"[AzuAudio] Tuile {tile.name} -> {tileSurface} (database{(isRetry ? ", apres refresh du cache" : "")})");
            ...
            {
                if (logSurfaceDetection)
                    Debug.Log($"[AzuAudio] Tuile {tile.name} -> {tilemapTag.surface} (tag de {tilemap.name}{(isRetry ? ", apres refresh du cache" : "")})");

                surface = tilemapTag.surface;
                return true;
            }
    ...
    {
        if (tilemapCache.TryGetValue(collider, out Tilemap[] cached) && IsCacheUsable(cached))
            return cached;

        Tilemap own = collider.GetComponent<Tilemap>();
        Tilemap[] found = own != null ? new Tilemap[] { own } : collider.GetComponentsInChildren<Tilemap>(true);

        if (found.Length == 0 && collider.transform.parent != null)
            found = collider.transform.parent.GetComponentsInChildren<Tilemap>(true);

        tilemapCache[collider] = found;
        return found;
    }

    bool IsCacheUsable(Tilemap[] cached)
    {
    ...
    SurfaceBank GetBankOrFallback(SurfaceType surface)
    {
        SurfaceBank bank = GetBank(surface);
        if (bank != null) return bank;

        bank = GetBank(defaultSurface);

        if (bank == null)
            bank = GetFirstUsableBank();

        if (bank == null && missingBankLogged.Add(surface))
            Debug.LogError($"[AzuAudio] Aucune Surface Bank exploitable pour {surface} : les pas seront muets.");
        else if (bank != null && missingBankLogged.Add(surface))
            Debug.LogWarning($"[AzuAudio] Pas de Surface Bank pour {surface}, repli sur {bank.surface}.");
    ...
    SurfaceBank GetBank(SurfaceType surface)
    {
        ...
        {
            if (surfaceBanks[i] != null && surfaceBanks[i].surface == surface)
                return surfaceBanks[i];
        }

        return null;
    }

    SurfaceBank GetFirstUsableBank()
    {
        if (surfaceBanks == null) return null;

        for (int i = 0; i < surfaceBanks.Length; i++)
        {
            if (surfaceBanks[i] != null && surfaceBanks[i].footsteps != null && surfaceBanks[i].footsteps.Length > 0)
                return surfaceBanks[i];
        }
